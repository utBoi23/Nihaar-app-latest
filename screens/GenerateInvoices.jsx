...
const GenerateInvoice = () => {
  ...
  const fetchProductDetails = async (id) => {
    try {
      ...
      if (productDoc.exists) {
        ...
        const priceWithCommission = productData.price + productData.commission;
        return { id, ...productData, priceWithCommission }; // Include price + commission
      } else {
  ...
  const handleQRCodeScanned = async (e) => {
    ...
    if (product) {
      setScannedProducts((prevProducts) => [
        ...prevProducts,
        { ...product, qty: '1' } // Default quantity to 1
      ]);
    }
    ...
    try {
      ...
      const salesInfo = {
        ...
        products: scannedProducts.map(product => ({
          ...
          name: product.name,
          qty: product.qty,
          price: product.priceWithCommission // Save price + commission
        })),
        timestamp: new Date().toISOString(),
      };
      await addDoc(salesInfoRef, salesInfo);  // Add a new document to the "salesInfo" collection
      console.log("Sales info saved successfully!");
    } catch (error) {
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const totalAmount = scannedProducts.reduce((sum, product) => sum + product.priceWithCommission * product.qty, 0); // Use priceWithCommission
          ...
          <div class="invoice-details">
            ...
            <table>
              ...
              ${scannedProducts.map(product => `
                <tr>
                  <td>${product.name}</td>
                  <td>Rs. ${product.priceWithCommission.toFixed(2)}</td>
                  <td>${product.qty}</td>
                  <td>Rs. ${(product.priceWithCommission * product.qty).toFixed(2)}</td>
                </tr>
              `).join('')}
            </table>
          </div>
      ...
      // Save sales info
      await saveSalesInfo(invoiceNo, totalAmount); // Call saveSalesInfo here
        ...
        <View style={styles.container}>
          ...
          {scannedProducts.map((product, index) => (
            <View key={product.id} style={styles.productContainer}>
              ...
              <Text style={styles.productText}>Price: Rs. {product.priceWithCommission.toFixed(2)}</Text>
              <TextInput
                value={product.qty}
                onChangeText={(qty) => {
